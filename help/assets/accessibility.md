---
title: Toegankelijke eigenschappen en interfaces van  [!DNL Adobe Experience Manager Assets]
description: Weet hoe de toegankelijkheidseigenschappen in  [!DNL Adobe Experience Manager]  6.5 LTS  [!DNL Assets]  hulpgebruikers met handicaps.
...
| Het dialoogvenster wordt geopend en heeft de focus | `Esc` | Sluit het dialoogvenster. |
...
Gebruikers kunnen het toetsenbord gebruiken om naar het aanmeldingsveld te navigeren en dit in te vullen. Schermlezers melden foutberichten op de aanmeldingspagina telkens wanneer een gebruiker een onjuiste combinatie van gebruikersnaam en wachtwoord invoert.

Nadat u zich hebt aangemeld, kunnen DAM-gebruikers met een toetsenbord navigeren binnen de gebruikersinterface van [!DNL Assets] . U kunt met het toetsenbord navigeren naar de elementen van de gebruikersinterface, zoals linkerspoor, menu&#39;s, gebruikersprofiel, zoekbalk, bestanden en mappen en instellingen voor beheer en configuratie. De volgorde van de toetsenbordnavigatie is van links naar rechts en van boven naar beneden. Wanneer gebruikers met een toetsenbord navigeren, markeert de gebruikersinterface de optie voor actioneren met focus, met een verbeterd kleurcontrast en schermlezers vertellen het van commentaar. Waar van toepassing kondigen schermlezers de status (bijvoorbeeld uitgevouwen, samengevouwen of gemengd) van de menuopties met focus aan. De schermlezer kondigt ook het doel van de optie waarop actie kan worden uitgevoerd aan in plaats van bijvoorbeeld de weergave of interfacepositie.
...
* Een schermlezer kondigt tekstopties aan die het doel of de functionaliteit van de pictogrammen in plaats van hun namen weergeven.
* Gebruikers kunnen de interactieve gebruikersinterfaceopties in de lijst Verwijzingen met behulp van toetsenbordtoetsen openen en activeren.
...
* Wanneer u het toetsenbord gebruikt om te bladeren, hebben de gemarkeerde opties voor een actiefunctie een prominentere visuele focus met verbeterd contrast. Hierdoor wordt het gefocuste gebied beter herkenbaar voor de gebruiker.
...
* De optie [!UICONTROL Save & Close] op de pagina Asset [!UICONTROL Properties] kan nu worden geopend met het toetsenbord.
...
* De paginatitel, beschikbaar voor een schermlezer, helpt de pagina te identificeren als een zoekpagina met middelen.
...
* Schermlezers kunnen selectievakjes met gemengde status in het deelvenster Filters identificeren en aankondigen wanneer gebruikers zoekresultaten filteren. In een gemengde staat, wordt het eerste niveaucontrolevakje geraakt door tot de gebruikers alle genestelde predikaten selecteren.
...
* Schermlezers kondigen voorspelden aan die opties voor gemengde frames bevatten.
...
* In het dialoogvenster voor het delen van koppelingen kunnen de schermlezers tijdens het navigeren in de bladermodus:
   ...
   * Navigeer naar alle vermelde suggesties.
...
Als u feedback wilt geven, vragen wilt stellen en productverbeteringen wilt aanvragen die betrekking hebben op toegankelijkheid, gebruikt u de volgende methoden. E-mail ons op `access@adobe.com` .
