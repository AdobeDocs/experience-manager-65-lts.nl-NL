...
>[!NOTE]
>
>De bestanden ImageMagick `policy.xml` en `configure.xml` zijn beschikbaar via `/usr/lib64/ImageMagick-&#42;/config/` in plaats van via `/etc/ImageMagick/` . Zie de documentatie ImageMagick (`https://www.imagemagick.org/script/resources.php` website) voor plaats van de configuratiedossiers.
