...
# Java API voor uitvoerservice, snel aan de slag (SOAP) {#output-service-java-api-quick-start-soap}
...
[Snel starten (SOAP-modus): een document in de AEM Forms Repository doorgeven aan de Output-service met de Java API](output-service-java-api-quick.md#quick-start-soap-mode-passing-a-document-located-in-the-repository-to-the-output-service-using-the-java-api)
...
[Snel starten (SOAP-modus): een afdrukstream naar een netwerkprinter verzenden met de Java API](output-service-java-api-quick.md#quick-start-soap-mode-sending-a-print-stream-to-a-network-printer-using-the-java-api)
...
Het volgende de codevoorbeeld van Java leidt tot een document van PDF genoemd *Loan.pdf*. Dit document van PDF is gebaseerd op een vormontwerp genoemd *Loan.xdp* en een het gegevensdossier van XML genoemd *Loan.xml*. *Loan.pdf* wordt geschreven aan de C:\Adobe omslag die op de J2EE toepassingsserver wordt gevestigd die AEM Forms, niet de cliëntcomputer ontvangt. (Zie [&#x200B; Creërend PDF Documenten &#x200B;](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents).)
...
Het volgende de codevoorbeeld van Java leidt tot een document van PDF genoemd *Loan.pdf*. Dit document van PDF is gebaseerd op een vormontwerp genoemd *Loan.xdp* en een het gegevensdossier van XML genoemd *Loan.xml*. Het XDP-bestand wordt geïmplementeerd als onderdeel van een AEM Forms-toepassing met de naam `Applications/FormsApplication` . Het URI-pad is `repository:///Applications/FormsApplication/1.0/FormsFolder/` . *Loan.pdf* wordt geschreven aan de C:\Adobe omslag die op de J2EE toepassingsserver wordt gevestigd die AEM Forms, niet de cliëntcomputer ontvangt. (Zie [&#x200B; Creërend PDF Documenten &#x200B;](/help/forms/developing/creating-document-output-streams.md#creating-pdf-documents).)

>[!NOTE]
>
>Voordat u deze snelle start uitvoert, moet u eerst een AEM Forms-toepassing met de naam Applications/FormsApplication maken. Maak een map in de toepassing met de naam FormsFolder en plaats het XDP-bestand in de map. Voor meer informatie, zie [&#x200B; een document van PDF &#x200B;](/help/forms/developing/creating-document-output-streams.md)*produceren.*
...
## Snel starten (SOAP-modus): een document in de gegevensopslagruimte doorgeven aan de uitvoerservice met de Java API {#quick-start-soap-mode-passing-a-document-located-in-the-repository-to-the-output-service-using-the-java-api}

De volgende Java-code haalt een XDP-bestand op uit de opslagplaats en geeft het bestand door aan de uitvoerservice in `com.adobe.idp.Document` -instantie. Het XDP-bestand wordt geïmplementeerd als onderdeel van een AEM Forms-toepassing met de naam `Applications/FormsApplication` . Het URI-pad is `repository:///Applications/FormsApplication/1.0/FormsFolder/` .

>[!NOTE]
>
>De Repository-API wordt gebruikt om het XDP-bestand van deze locatie op te halen. (Zie [&#x200B; Leesmiddelen &#x200B;](/help/forms/developing/aem-forms-repository.md#reading-resources).)

De hoofdwaarde van de inhoud `repository:///Applications/FormsApplication/1.0/FormsFolder/` wordt ook doorgegeven aan de methode `OutputClient` object `generatePDFOutput2` (de tweede parameter). Deze waarde wordt doorgegeven aan de service Uitvoer om de service Uitvoer te informeren dat formulierelementen, zoals afbeeldingen, op deze locatie worden opgeslagen.

>[!NOTE]
>
>U kunt de hoofdwaarde van de inhoud op dezelfde manier instellen wanneer u de methode `generatePrintedOutput2` aanroept.

*Loan.pdf* wordt geschreven aan de C:\Adobe omslag die op de J2EE toepassingsserver wordt gevestigd die AEM Forms ontvangt. (Zie [&#x200B; het overgaan van Documenten in de Bewaarplaats aan de Dienst van de Output &#x200B;](/help/forms/developing/creating-document-output-streams.md#passing-documents-located-in-the-repository-to-the-output-service).)
...
Het volgende de codevoorbeeld van Java leidt tot een document PDF/A genoemd *LoanArchive.pdf*. Dit document van PDF is gebaseerd op een vormontwerp genoemd *Loan.xdp* en een het gegevensdossier van XML genoemd *Loan.xml*. *LoanArchive.pdf* wordt geschreven aan de C:\Adobe omslag die op de J2EE toepassingsserver wordt gevestigd die AEM Forms, niet de cliëntcomputer ontvangt. (Zie [&#x200B; Creërend PDF/A Documenten &#x200B;](/help/forms/developing/creating-document-output-streams.md#creating-pdf-a-documents).)
...
Het volgende snelle begin van Java wint het dossier *Loan.xdp* van de Diensten van de Inhoud terug. Dit XDP-bestand bevindt zich in de map `space /Company Home/Form Designs` . Het XDP-bestand wordt geretourneerd in een `com.adobe.idp.Document` -instantie. De instantie `com.adobe.idp.Document` wordt doorgegeven aan de service Uitvoer. Het niet-interactieve formulier wordt opgeslagen als een PDF-bestand met de naam *Loan.pdf *op de clientcomputer. Omdat de optie File URI is ingesteld, wordt het PDF-bestand *Loan.pdf *ook opgeslagen op de J2EE-toepassingsserver die als host fungeert voor AEM Forms. (Zie [&#x200B; het overgaan van Documenten in Inhoud Services ES2 tot de Dienst van de Output &#x200B;](/help/forms/developing/creating-document-output-streams.md#passing-documents-located-in-content-services-deprecated-to-the-output-service).)
...
Het volgende codevoorbeeld van Java drukt een outputstroom aan een dossier van PostScript genoemd *MortgaugeForm.ps*. (Zie [&#x200B; DRUKKEN aan Dossiers &#x200B;](/help/forms/developing/creating-document-output-streams.md#printing-to-files).)
...
In het volgende Java-codevoorbeeld wordt een PostScript-afdrukstroom verzonden naar een netwerkprinter met de naam *\\Printer1\Printer* . Er worden twee exemplaren naar de printer verzonden. (Zie [&#x200B; verzendend de Streams van de Druk aan Printers &#x200B;](/help/forms/developing/creating-document-output-streams.md#sending-print-streams-to-printers).)
...
De volgende code van Java leidt tot veelvoudige dossiers van PDF voor elk gegevensverslag dat in een dossier van XML genoemd *Loan_data_batch.xml* is. De bestanden worden naar de map C:\Adobe geschreven. De PDF-bestanden worden geschreven naar de map C:\Adobe op de J2EE-toepassingsserver die als host fungeert voor AEM Forms, niet naar de clientcomputer. (Zie [&#x200B; Creërend Veelvoudige Dossiers van de Output &#x200B;](/help/forms/developing/creating-document-output-streams.md#creating-multiple-output-files).)
...
In het volgende Java-codevoorbeeld worden twee tekstpatronen gemaakt waarnaar de uitvoerservice zoekt. Het eerste tekstpatroon is Hypotheek. Indien gevonden, gebruikt de dienst van de Output het vormontwerp genoemd *Mortgauge.xdp*. Het tweede tekstpatroon is Automobile. Indien gevonden, gebruikt de dienst van de Output het vormontwerp genoemd *AutomobileLoan.xdp*. Als geen van beide tekstpatronen wordt gevonden, gebruikt de Output-service het standaardformulierontwerp met de naam* Loan.xdp. *(Zie [&#x200B; Creërend de Regels van het Onderzoek &#x200B;](/help/forms/developing/creating-document-output-streams.md#creating-search-rules).)
...
Het volgende de codevoorbeeld van Java transformeert een interactief document van PDF genoemd *Loan.pdf* aan een niet-interactief document van PDF genoemd *NonInteractiveLoan.pdf*. (Zie [&#x200B; het Afvlakken de Documenten van PDF &#x200B;](/help/forms/developing/creating-document-output-streams.md#flattening-pdf-documents).)
