...
Als er zich meerdere handtekeningvelden in een PDF-document bevinden, is het raadzaam de volledige namen van de handtekeningvelden op te geven. Dat wil zeggen, geef `form1[0].Form1[0].SignatureField1[0]` op in plaats van `SignatureField1[0]` .
...
1. Het interactieve formulier ophalen met de Forms-service
   ...
   * Maak een `java.io.FileInputStream` -object dat het PDF-document vertegenwoordigt dat aan de Forms-service moet worden doorgegeven met behulp van de constructor ervan. Geef een tekenreekswaarde door die de locatie van het PDF-document aangeeft.
   * Maak een `com.adobe.idp.Document` -object door de constructor ervan te gebruiken en het `java.io.FileInputStream` -object door te geven.
...
1. Het PDF-document certificeren
   ...
   * Een tekenreekswaarde die de locatie van de ondertekenaar vertegenwoordigt.
   * Een tekenreekswaarde die de contactgegevens van de ondertekenaar vertegenwoordigt.
   * Een `MDPPermissions` -object met statische gegevens dat handelingen opgeeft die kunnen worden uitgevoerd op het PDF-document waardoor de handtekening ongeldig wordt.
...
**krijgt het document van PDF dat de te verifiëren handtekening bevat**

Als u een handtekening waarmee een PDF-document digitaal wordt ondertekend of gecertificeerd, wilt verifiëren, vraagt u een PDF-document met een handtekening aan.
...
U kunt de de serverorde plaatsen CRL en OCSP die de dienst van de Handtekening door de Toepassingen en de Diensten van Adobe te gebruiken gebruikt. Bijvoorbeeld, als de OCSP server eerst in de Toepassingen en de Diensten van Adobe wordt geplaatst, dan wordt de server OCSP gecontroleerd, die door de server CRL wordt gevolgd.
...
1. Het PDF-document ophalen dat de handtekening bevat die moet worden geverifieerd
...
1. Inclusief projectbestanden.
1. Maak een handtekeningclient.
1. Hiermee wordt het PDF-document met de te verifiëren handtekeningen opgehaald.
1. Stel PKI-runtime-opties in.
...
**krijgt het document van PDF dat de te verifiëren handtekeningen bevat**
...
U kunt de de serverorde plaatsen CRL en OCSP die de dienst van de Handtekening door de Toepassingen en de Diensten van Adobe te gebruiken gebruikt. Bijvoorbeeld, als de OCSP server eerst in de Toepassingen en de Diensten van Adobe wordt geplaatst, wordt de server OCSP gecontroleerd, die door de server CRL wordt gevolgd.
...
1. Het PDF-document ophalen dat de handtekeningen bevat die moeten worden gecontroleerd
...
1. Inclusief projectbestanden.
1. Maak een handtekeningclient.
1. Hiermee wordt het PDF-document opgehaald dat een handtekening bevat die moet worden verwijderd.
...
**krijgt het document van PDF dat een handtekening bevat om te verwijderen**
...
1. PDF-document ophalen dat een handtekening bevat die moet worden verwijderd

   * Maak een `java.io.FileInputStream` -object dat staat voor het PDF-document dat de te verwijderen handtekening bevat met behulp van de constructor ervan en geef een tekenreekswaarde door die de locatie van het PDF-document aangeeft.
   * Maak een `com.adobe.idp.Document` -object door de constructor ervan te gebruiken en het `java.io.FileInputStream` -object door te geven.
