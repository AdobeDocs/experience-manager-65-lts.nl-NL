...
description: U kunt een PDF-portfolio samenstellen om verschillende soorten documenten te combineren, zoals tekstbestanden, afbeeldingsbestanden en PDF-documenten. U kunt een PDF-portfolio samenstellen met behulp van een Java API en een webservice-API.
contentOwner: admin
...
Wanneer u een PDF Portfolio samenstelt, geeft u een NAV-bestand (een navigatorbestand) door aan de Assembler-service. Het NAV-bestand dat u doorgeeft aan de Assembler-service, is afhankelijk van het type PDF Portfolio dat u wilt maken. Bijvoorbeeld, om een *op een lay-out van het Beeld* tot stand te brengen, ga het dossier over AdobeOnImage.nav. U kunt NAV-bestanden zoeken in de volgende map:
...
>[!NOTE]
>
>Met AdobeOnImage.nav kunt u snel aan de slag met het samenstellen van PDF-portfolio&#39;s.
...
Als u een PDF Portfolio wilt samenstellen, roept u de bewerking `invokeDDX` aan. De service Assembler retourneert de PDF Portfolio binnen een verzamelingsobject.
...
Stel een PDF Portfolio samen met de Assembler Service API (Java):
...
1. De portfolio samenstellen.
   ...
   * Een `java.util.Map` -object dat de bestanden bevat die vereist zijn om een PDF Portfolio te bouwen.
...
1. Sla het samengevoegde portfolio op.
   ...
   * Roep de methode `copyToFile` van het `com.adobe.idp.Document` -object aan om de PDF Portfolio te extraheren.
...
Stel een PDF Portfolio samen met behulp van de Assembler Service API (webservice):
...
1. Verwijs naar de vereiste documenten.
   ...
   * Wijs het `BLOB` -object toe dat het invoerbestand opslaat in het veld `MyMapOf_xsd_string_To_xsd_anyType_Item` Object `value` . (Voer deze taak uit voor elk PDF-invoerdocument.)
