...
# Over de Universal Editor {#universal-editor}
...
De Universal Editor is een veelzijdige visuele editor die deel uitmaakt van Adobe Experience Manager Sites. Auteurs kunnen hiermee &#39;what-you-see-is-what-you-get&#39; (WYSIWYG)-bewerkingen uitvoeren op elke headless-ervaring.
...
* De Universal Editor leest de AEM-pagina via de Dispatcher vanuit de AEM-auteur-instantie.
...
Het volgende biedt ondersteuning voor de Universal Editor:
...
Ga als volgt te werk om de Universal Editor te gebruiken:

1. [Configureer services op uw AEM-ontwerpinstantie.](#configure-aem)
1. [Stel een lokale Universal Editor-service in.](#set-up-ue)
1. [Pas uw Dispatcher aan om de Universal Editor Service toe te staan.](#update-dispatcher)
...
#### De Adobe Granite Query Parameter Authentication Handler configureren {#query-parameter}
...
#### Definieer welke inhoudspaden of `sling:resourceTypes` open zijn in de Universal Editor {#paths}
...
1. Definieer voor welke inhoudspaden of `sling:resourceTypes` de universele editor moet worden geopend.
   ...
   * In het **Verdelen :resourceTypes dat door het Universele gebied van de Redacteur** zal worden geopend, ga een lijst van middelen in die de Universele Redacteur direct opent.
1. Klik **sparen**.
1. Controleer uw [&#x200B; configuratie ExternalAlizer &#x200B;](/help/sites-developing/externalizer.md) en zorg bij een minimum dat u de lokale, auteur, en publiceer milieu&#39;s hebt die zoals in het volgende voorbeeld worden geplaatst:
...
Zodra deze configuratiestappen volledig zijn, opent AEM de Universele Redacteur voor pagina&#39;s in de volgende orde:
...
1. Voor inhoud buiten de wegen die in `Universal Editor Opening Mapping` worden bepaald, controleert AEM of de inhoud `resourceType` een ingang in **het Verdelen :resourceTypes aanpast die door Universele Redacteur** zal worden geopend. Als dit overeenkomt, opent AEM de inhoud in de Universal Editor op `${author}${path}.html` .
...
* Open alle pagina&#39;s onder `/content/foo` op de AEM-auteur:
   ...
   * Resultaten bij openen `https://localhost:4502/content/foo/x.html?login-token=<token>`
* Open alle pagina&#39;s onder `/content/bar` op een externe NextJS-server, waarbij alle variabelen als informatie worden opgegeven
   ...
   * Resultaten bij openen `https://nextjs.server/content/bar/x?env=prod&author=https://localhost:4502&publish=https://localhost:4503&login-token=<token>`
...
## Uw app instrumenten {#instrumentation}
...
De Universal Editor in AEM 6.5 LTS werkt in grote lijnen hetzelfde als in AEM as a Cloud Service, inclusief de gebruikersinterface en een groot deel van de installatie. Er zijn echter verschillen die u moet opmerken.
